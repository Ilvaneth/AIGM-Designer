...
- **Status:** slices 0, 1a and 1b delivered (plan 24.3; 1b on 2026-09-25: the 25 `data/design/*.yaml` tables, `design_tables.py`, `build_design_index.py` → `srd-index-2014.json`); next is slice 1c, prompts, Workflows and the approval loop, which needs the user's decision on auto-approve for test births. The fixture bible is `.claude/skills/dnd/tests/fixtures/salt-lantern/`, the schemas `docs/schemas/`.
