...
- **Status:** slices 0, 1a and 1b delivered (plan 24.3; 1b on 2026-09-25: the 25 `data/design/*.yaml` tables, `design_tables.py`, `build_design_index.py` → `srd-index-2014.json`); slice 1c items 1-7 delivered 2026-09-25 (`designer.py`, the prompt library, `design_approval.py`, `design_revise.py`, `render_player.py`, the two Workflows under `.claude/workflows/`, `SKILL-design.md`); item 8 in progress: tuning birth 1 ran 2026-09-25 in an Opus tab (`docs/tuning-births.md` is the protocol, `docs/reports/tuning-birth-1.md` the report), birth 2 ran the same evening; both reports' findings are fixed and pinned by `tests/test_tuning_birth_1.py` / `test_tuning_birth_2.py`; birth 2 resumed 2026-09-26 and reached P8 with the resume and leak tests passed; the addendum's findings are fixed. Slice 1d (play integration: `map_travel.py`, `render_dm.py`, `play_pack.py` via `designer.py load-pack / scene / prep / detail / spotlight / end-pack`, `burst_check --reference`, play registration, the SKILL edits) delivered 2026-09-26. Slice 1e tooling delivered 2026-09-26 (`design_leak_scan.py`, `design_compare.py`, `load_budget.py`, `playtest.py` + the `player` agent + the judges under `prompts/play/`; protocol `docs/dry-run-and-playtest.md`); dry run 1 ran 2026-09-26 (report `docs/reports/dry-run-1.md`: P0-P9, character, ten-turn playtest, judges; findings fixed); critique analysis 1 (2026-09-26, `docs/reports/critique-analysis-1.md`) moved the leak, naming and stub classes from the critics to the registry door; part D (`_test-dry-2`, the uniqueness check) is running in the Opus tab on the new door, then slice 2. Then slice 1d, play integration. The fixture bible is `.claude/skills/dnd/tests/fixtures/salt-lantern/`, the schemas `docs/schemas/`.
