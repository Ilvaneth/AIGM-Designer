...
1. **Size HP for multiple rounds, not for round one.** Take the realistic opening burst — in a designed campaign the **reference party's** (`burst_check.py --campaign <name> --reference --tier <site tier> --target-hp <HP>`: the tier's intended level, never the actual party, because the world does not scale; for a hand-built fight at the table, `burst_check.py` reads the real sheets) and add two rounds of sustained DPR on top. A boss that survives the alpha strike but dies in round two produced the same anticlimax.
