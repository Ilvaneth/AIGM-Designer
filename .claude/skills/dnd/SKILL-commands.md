...
0.5. **Decide whether this is a new session — driven only by an explicit `/dm:dnd end`, never by the real-world calendar (changed 2026-09-14 after the date-based version kept incrementing sessions on reloads the table didn't consider "new").** Read `state.md → ## Session Flags` for `session_status`. Two cases:
...
If nothing in this session touched a tracked NPC/faction's goal, this step is a fast no-op — don't manufacture updates.
