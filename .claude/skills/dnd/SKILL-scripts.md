...
## Design Tables — `data/design/*.yaml`, `scripts/design_tables.py`

The designer's creative DNA (plan item 17): `dials.yaml` (the closed dial lists and each value's downstream effects), `scale.yaml` (every count by scale, the arc shape, room bands, the stacking rules, the volatility budget), `tensions.yaml`, `secrets.yaml`, `trope-breaks.yaml`, `forbidden.yaml`, `signatures.yaml`, `naming.yaml`, and the later batches (cosmology, lands, powers, people, sites, arc, threads, rubrics). Every row has `id` (unique across all tables), `label` and `hooks` (`{phase, must}`: how the row must propagate); `weight` is optional. `design_tables.py` is the one reader: `rows("dials.yaml#tone")` addresses a sub-table, `design_dice.py roll --table` draws through it, `design_manifest.py` takes its dial lists and `ARC_SHAPE` from it, and `design_tables.py list | rows REF | show REF ROW_ID` inspect the files. Tables are English skill data; every name they seed is an English-language fantasy name carried into Turkish prose with an apostrophe suffix (errata 24.2 #17), and `naming.yaml` bans exact names only, never word stems (#18). `tests/test_design_tables.py` enforces the convention, the plan's numbers and the size floors.

## Designer — `scripts/designer.py`, `scripts/design_prompts.py`, `prompts/design/`

`designer.py` is the conductor's CLI, the deterministic half of every designer command (plan item 19, 21.E): `new NAME --scale … --party-size N` rolls blank dials live, initialises the manifest, writes the Phase 0 card and arms the read guard; `preroll --phase PN` makes every labelled, seeded roll a phase needs from the tables (secret rolls to dm-only); `phase PN begin --json` reconciles, arms and prints the roster with read budgets and, per entity, the prompt's render command and a rendered copy under `design/_prompts/`; `phase PN merge` runs `registry.py merge` and `design_seed.py` and absorbs the skeleton agent's `skeleton.json` (roster, label → id assignments, reads); `check`, `card`, `approve` (auto for `--fixture` and `_test-*` campaigns unless `new` was given `--ask-approval`, `--onay` otherwise, a path-scoped `design(<campaign>):` commit — never for a git-ignored test campaign), `rerun`, `status`, `abandon`, `drop --id ID --reason` (a roster item that never reached the registry). `merge` is per unit — a refused fragment is marked `failed` with its reason and the next `begin --json` renders it one attempt later — and takes `--tokens N --seconds S` from the Workflow result for the card; `approve --force` accepts an incomplete roster; `check --full` prints every validator line instead of the grouped summary; every render command carries `--phase PN` so the all-phase critic prompts (`critic`, `phase_critic`, `wishes_critic`, `skeleton_critic`) render for their phase. `design_prompts.py` renders `prompts/design/*.md` (front matter: phase, role, kind, effort, critics, schema; placeholders from the manifest, the public projection, the rolls, the read budget, the rubric rows, the shared `_common.md` preamble) and `check`s the library; an agent may render its own prompt with the command `begin` prints. Returns follow `prompts/design/schemas/*.json`: ids, statuses, paths and counts, never prose.

## Phase Cards — `scripts/design_approval.py`

`design_approval.py -c CAMP card --phase PN` writes the Turkish phase card the owner approves from (plan item 19.6): counts against the scale band (computed script-side with the full count, printed as the public count and a tick), the wishes' per-wish ticks from the wishes critic, the validator summary in redacted form, critique loops and failed ids, tokens and minutes, the public names with their naming language and one line each, a "what a native knows" excerpt, the text form of the player map on the lands card, the spoiler-safe abstract of the secret layer on the premise card (archetype class, novelty against `used.json`, clue count per act, critic agreement) and, on a rerun, the diff against the previous card, which is archived. Before it is written the card is scanned for every secret entity's name and every dm-only sentence; a hit refuses it (`leak-check FILE` runs the same scan on anything). `critique --phase PN --file RETURN.json [--critic N]` stores a critic's return as ids, verdicts and reason codes only and counts the fix loops; a return carrying free text is refused. `designer.py phase PN card` and `approve` use it.

## Birth-round Revise — `scripts/design_revise.py`

During phase approval a correction is one sentence and revise is free (plan item 14.3, 19.6). The conductor classifies it and runs `design_revise.py -c CAMP round --phase PN --scope direction|fact|entity|phase --text "…"`: a **direction** is stored on the phase (or on the wishes with `--world`) and marks the named entities, or the whole roster, for a rerun; a **fact** (`--entity ID --field F --value V`) changes one public registry field, moves the projection and the birth snapshot with it, and marks the entity and every referrer for a rerun; an **entity** round removes one (prose archived under `design/_revised/`) or marks it for replacement; a **phase** round reruns the phase through `designer.py` (`--reseed` for a new master seed). Every round lands on the phase's approval record (three at most, then approve-as-is or a phase rerun) and in the revision log; `affected --entity ID` lists the public referrers and counts the hidden ones. A rerun-marked entity stays pending until a fresher fragment appears, whatever the disk says. Secret entities and dm-only fields are slice 3's `revise --dm-only --role`.

## Player Files — `scripts/render_player.py`

The player-facing files, built from the public projection only (plan item 11.3-11.6, 16.2): `primer` assembles `design/player-primer.md` (the pitch; per polity the P8 section file from staging plus the registry-built headings: the land and who rules it, the gods as worshipped, calendar and festivals, money and prices, languages and peoples, magic and its keepers, famous places, history as taught, what everyone says is dangerous from the over-tier sites' far telegraphs, the day-0 rumours from `news.json`; then the sockets' questions and the text player map); `facts` merges the P8 `*.facts.json` fragments into `common-knowledge.json`; `news` merges the `*.news.json` fragments into `news.json` as the day-0 slice; `thread-face THREAD` writes a thread's public section to `design/player/`; `resolve FILE` rewrites `[[id]]` links to names; `check FILE` runs the leak scan. A link to a discoverable or secret entity, a secret name, a hidden id or a dm-only sentence refuses the file.

## Play Integration — `scripts/map_travel.py`, `scripts/render_dm.py`, `scripts/play_pack.py` (designed campaigns)

`map_travel.py -c CAMP travel-times` writes `design/travel-times.md` from `design/map.json` (shortest paths from each region's hub, the settlement matrix, the routes; secret nodes only in a dm-only copy); `encounters` compiles the region files' `### Travel table` sections into `reference/travel-encounters.md` for `travel.py` (the Roll Log is kept); `days A B` and `near ID [--days 1] [--kind site] [--json]` answer reachability. `render_dm.py -c CAMP world | npcs | index | state [--force] | report | all` generates `world.md`, `npcs.md`, `design/index.md`, a lean `state.md` (only when missing) and `design/report.md` from the public projection ⊕ overlay, every overlaid value with its *(değişti …)* marker. The play verbs of `designer.py` live in `play_pack.py`: `load-pack [--day N --session N --intent T --json]`, `scene --enter ID [--hours N --present a,b --json]`, `prep [--day N --level L --intent T --max 3 --note T --json]`, `detail ID [--trigger prep|approach|hand --day N --json]` / `detail ID --finish --day N`, `spotlight --session N --scenes thread_a=3,thread_b=1`, `end-pack --day N --session N [--intent T --note T]`. `burst_check.py --campaign CAMP --reference --tier T [--size N --level L] --target-hp H` is the reference-party mode (no sheets read).

## Dry Run and Playtest — `scripts/design_leak_scan.py`, `design_compare.py`, `load_budget.py`, `playtest.py` (development)

The test layers of plan item 22 (slice 1e; protocol `docs/dry-run-and-playtest.md`). `design_leak_scan.py -c CAMP [--extra FILE …] [--journal DIR] [--json]` greps every dm-only sentence and secret name against every conductor-visible artifact and reports hits by length, never by text (exit 1 on a hit). `design_compare.py A B [--json]` sets two births against each other: shared names, shared rows of the unique tables (tensions, secrets, trope breaks, signatures, naming families, pantheon type), map shape, demographics, and prints both premises' public lines for the uniqueness judge (exit 1 when not distinct). `load_budget.py -c CAMP [--budget-tokens 40000]` sums what `load` reads. `playtest.py -c CAMP start --pc NAME --persona TEXT` arms the guard in playtest mode with the player agent's allowlist (the primer, the PC's sheet, the thread faces, the transcript); `turn --dm TEXT` records the narration and prints the player agent's prompt (`prompts/play/player.md`; spawn it with the Agent tool as `subagent_type: player`, defined in `.claude/agents/player.md`); `turn --player TEXT` records the answer; `judge playtest | readability | uniqueness [--against CAMP2]` prints a judge's prompt and `--record FILE` stores its return; `stop` disarms. The player agent rolls its own PC's die with `dice.py <notation> --owner PC --player`, which `dice.py` and `dice_guard.py` allow only for a playtest agent while a playtest is armed.

## Critique Statistics — `scripts/design_critique_stats.py` (development)

`design_critique_stats.py CAMP [CAMP …] [--json] [--top N]` reads every critic return kept under `design/_staging/<PN>/merged/` (ids, verdicts, rubric ids, reason codes — never prose) and each manifest's cost: first and final verdicts, fix loops used and whether a `fix` converted, second-critic disagreement, the rubrics that fail most with their reason codes, the rubrics that never fail, tokens and minutes per phase and per roster item. Run it after every birth; `docs/reports/critique-analysis-1.md` is the first reading.

**The registry door** (`registry.py merge`, since critique analysis 1) refuses with the reason in the retry prompt: a stamped change without `--revise`, a mirror sentence contained in the public file or the public notes, a secret name or a `## Secret` heading outside dm-only (the public notes included), a public file without `mirror:` or a mirror without `secrecy: secret` / `mirror_of`, a prose-type row without a file that is not a `status: pending` stub, a secret npc without an opaque id, a name on `naming.yaml`'s blacklist (mythology, the owner's names, the model's favourites, stems, substrings, `turkish_as_name`), Turkish letters in a name (breaks and the premise excepted), a name another campaign registered, duplicate full names within persons / places / factions / gods / items / planes and first-name collisions among persons. The projection never carries a secret id.

## SRD Index — `scripts/build_design_index.py` → `data/design/srd-index-2014.json`, `monster-ecology.yaml`

The SRD in the shape the designer queries (plan item 18.1): every monster (the 334 of the built JSON, joined with the YAML appendices for immunities, resistances, legendary actions and spellcasting) with its ecology row; the 362 magic items by rarity, category and attunement; the 319 spells by class and level with a planar-reference flag; the Running-the-Game hazards (eight traps with DCs, three diseases, fourteen poisons, the three madness tables); backgrounds, languages, equipment and the trade-goods table. The gods appendix is never read (errata 24.2 #19). Candidate lists are filtered from the index script-side before the LLM chooses and reskins (item 18.2). `monster-ecology.yaml` is generated by the same script: heuristic v0 rows (`curated: false`) from type, name and speeds, and hand-curated rows (`curated: true`, the NPC appendix and the staples) that survive a rebuild. `build_design_index.py --check` fails when either file is stale; `--stats` prints the counts.
