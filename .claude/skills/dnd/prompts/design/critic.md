...
The overall verdict is `rerun` if any row says rerun, `fix` if any row says fix, else `pass`. Two fix loops are all the phase gets; be exact about what must change. Save your return as `design/_staging/{{phase}}/{{entity_id}}.critic{{critic_order}}.json` before returning it: `designer.py phase merge` records it from there.
