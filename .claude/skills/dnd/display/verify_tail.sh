...
# Resolve the campaign root the same way paths.py does: DND_CAMPAIGN_ROOT is
# the DATA root (campaigns live under <root>/campaigns/<name>/), not the
# campaigns folder itself — appending "/campaigns" here is required, the same
# way campaigns_dir() does it in paths.py. Falls back to the legacy default
# (~/.claude/dnd/campaigns) when the env var is unset.
if [[ -n "${DND_CAMPAIGN_ROOT:-}" ]]; then
  ROOT="${DND_CAMPAIGN_ROOT}/campaigns"
else
  ROOT="$HOME/.claude/dnd/campaigns"
fi
