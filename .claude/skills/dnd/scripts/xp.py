...
def _monster_multiplier(count: int, players: int = 4) -> float:
    """DMG encounter multiplier by monster count, bumped one step for a
    small party. A party smaller than 3 has no spare action economy to
    absorb a group fight the way a standard 4-person party does — DMG p.82
    calls for exactly this adjustment ("increase the multiplier by one step
    if the party has fewer than three members"), and it was previously a
    comment-only intention here, never actually wired to `players`."""
    idx = None
    for i, (threshold, _mult) in enumerate(MONSTER_MULTIPLIERS):
        if count <= threshold:
            idx = i
            break
    if idx is None:
        idx = len(MONSTER_MULTIPLIERS) - 1
    if players < 3:
        idx = min(idx + 1, len(MONSTER_MULTIPLIERS) - 1)
    return MONSTER_MULTIPLIERS[idx][1]
...
def _read_char_state(path: pathlib.Path) -> tuple[int, int]:
    ...
    Two sheet conventions are supported:
      - Markdown table row (this skill's own character-sheet.md template):
            | **Experience Points** | 29938 / 34000 |
            | **Level** | 7 |
      - Inline bold-colon style (an older/alternate convention):
            **XP:** 1234 / 2700
            **Level:** 5
    Table format is tried first since it's the template this skill ships;
    inline is a fallback for sheets written to the older convention.
    """
    text = path.read_text(encoding="utf-8")

    xp_m = re.search(
        r"\|\s*\*\*(?:Experience Points|XP)\*\*\s*\|\s*(\d+)\s*/\s*\d+[^\n|]*\|",
        text,
    ) or re.search(r"\*\*(?:XP|经验|经验值):\*\*\s*(\d+)", text)

    level_m = re.search(
        r"\|\s*\*\*Level\*\*\s*\|\s*(\d+)\s*\|", text
    ) or re.search(r"\*\*(?:Level|等级):\*\*\s*(\d+)", text)
...
def _write_char_xp(path: pathlib.Path, new_xp: int, current_level: int) -> bool:
    ...
    # Try the markdown-table row first (this skill's own character-sheet.md
    # template): | **Experience Points** | 29938 / 34000 |
    ...
    if hits == 0:
        # Fall back to the older inline bold-colon convention:
        # **XP:** 1234 / 2700  (also handles 经验/经验值 for that convention's
        # localized variant)
    ...
    if hits == 0:
        # Fresh template sheets, or a sheet using neither known convention —
        # a regex mismatch would silently skip writing; warn explicitly
        # instead of pretending the award succeeded.
