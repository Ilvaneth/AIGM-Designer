...
Then branch to the matching procedure in `SKILL-commands.md` (`/dm:dnd load`, `/dm:dnd new`, `/dm:dnd design …`, `/dm:dnd import`, `/dm:dnd character …`); `new` and `design` also load `SKILL-design.md`.
...
**Per-campaign calibration lives in `state.md → ## DM Style Notes`.** Read it at every load. It holds what is true of *this* table and no other: the party's own conventions, a player's personal notation, which threads need balancing, what this group responded well to. Update it at `/dm:dnd end` when new patterns emerge. This is the mechanism that makes Standard 9 compound across sessions rather than resetting each time.

**A general rule does not belong there.** When a table states a rule that would hold for any campaign — how dice are owned, what an NPC can know, how a fight is narrated — write it into this skill and delete whatever it contradicts, in the same change. A general rule filed as a campaign-level "override" loses every time it meets the skill text at the moment of play: the skill is loaded first, carries the worked examples, and is what the tools implement. Campaign files hold facts; the skill holds rules.
...
**Before building any encounter that is not random texture, read `SKILL-encounter-design.md`** — boss HP sized across rounds, resistance profiles by default, terrain protection, resource drain before the boss room, support creatures that survive a hit, answers to attack-roll shutdown, and enemy itemisation. "The fight was too easy" is a structural problem, and that file names each cause and its fix.
...
Between sessions, active factions and NPCs don't stand still waiting to be found. A faction move the party didn't prevent should show up as a visible change in the world — a rumour they hear, a door that's now locked, a face that's no longer in the market. The player doesn't need to know why yet. They need to feel that the world has weight.

**Ask the forward question, not the backward one.** "What did they do while the party was occupied?" is past tense, and a past-tense question can only produce a reaction to the party — which is how a campaign ends up with factions that feel like scenery. The question for each active faction is: **what is their next step, by when, and what would make them abandon it?** That is a plan, and a plan produces moves the party has to respond to instead of the other way round.

**The board is data, not memory.** `scripts/factions.py` holds every active faction's objective, running operation (steps with in-world due days), weekly move-point budget, intelligence rating, and stance toward every other faction and the party. Consult it rather than reconstructing who-opposes-whom from prose; prose cannot be queried at the moment a move is needed, which is exactly when it is needed.

- **When something changes the balance — an alliance, a death, a public humiliation, a captured asset — run `factions.py react`.** It names every faction that learns of it (filtered by how visible the event was and how good their intelligence is, with a delay), which of them are compelled to answer, and each one's standing doctrine and current plan. Write a move for each compelled faction before the session moves on. This is the mechanism behind "the party allies with one house and the others adjust": the adjustment is computed from stances, not remembered.
- **A move costs.** Each faction has a weekly budget of move points scaled to its power; acting spends them, and assets the party destroys reduce the budget. Standing still is then a real choice with a real cost, rather than the cheapest option available to a DM in a hurry.
- **Clocks tick with the calendar.** Operation steps carry due days; `factions.py tick --day N` refreshes budgets and flags what is due or overdue. Run it whenever the in-world date rolls over.
- **Plans outlive sessions.** An operation step that the party never noticed still resolves on its due day. That is what makes the world feel like it was running before they arrived.
...
### 15. The World Does Not Scale
A place has the danger its fiction gives it, fixed when the campaign was born and stamped in its registry row; nothing about a site, a region or a boss is derived from the party's level, and nothing is softened when they arrive under-levelled. What the world owes them instead is **legibility and a way out**: every site carries three telegraphs (a rumour a day away, a sign on the approach, the door itself) and an escape geometry, and a level-3 party that walks into a CR 15 site was told, three times, and can run. Intelligent monsters act by their own logic — the fiction, not the stats. Bosses are built against a reference party at the tier's intended level, never against the actual party (see `SKILL-encounter-design.md`). The designer stamps the tiers; the DM plays them straight.
...
${CLAUDE_SKILL_DIR}/                 ← the skill dir (plugin: <plugin>/skills/dnd/)
  ...
  SKILL-encounter-design.md ← how to build an encounter that does not collapse (load when designing one)
  ...
  SKILL-design.md    ← the Campaign Designer: dials, the phase loop, the design commands (load when `new` or a `design` command is invoked)
  prompts/design/    ← the designer's agent prompts and return schemas (rendered by scripts/design_prompts.py)
  ...
  data/design/       ← the designer's tables (25 YAML files), the SRD index and monster-ecology (generated by build_design_index.py)
  templates/         ← blank character-sheet.md, state.md, world.md, npcs.md, session-log.md
  templates/design/  ← the bible's file shapes (premise, cosmology, region, settlement, faction, npc, site, chapter, arc, thread, primer, report)
```

The designer's two Workflow scripts (`design-skeleton`, `design-fanout`) live in the project's `.claude/workflows/`, beside the skill, because the Workflow tool reads them from there.
...
<DATA root>/campaigns/<name>/
  ...
  design/            ← the bible (a designed campaign): entities.json (public projection), overlay.json, map.json, news.json, naming.json,
                        premise.md, cosmology.md, regions/, settlements/, factions/, npcs/, sites/, chapters/, threads/, arc.md,
                        player-primer.md, player/, _approval/ (cards), _staging/ and dm-only/ (agents only, never the conductor)
  factions.json / goals.json / graph.json / calendar.json / common-knowledge.json / site-progress.json
...
## The blind conductor — the Campaign Designer

`/dm:dnd new` is the designer (`SKILL-design.md`): a campaign bible is built phase by phase by fresh-context agents while the main session stays a **blind conductor**. The conductor asks the dials, runs `designer.py`, launches the `design-skeleton` and `design-fanout` Workflows, shows the script-built phase card and records approvals. It never writes bible prose and never reads `design/dm-only/**`, `design/_staging/**` or the canonical registry — it holds only the public projection and the redacted validator output — because the owner of this table is also its sole player, and what the conductor reads, the player reads. `hooks/design_read_guard.py` enforces the line while a designer command is armed; agents never roll dice (`dice_guard.py`), the conductor pre-rolls every labelled die with `designer.py preroll`. The bible is immutable in play except through `revise` (the player's canon change) and `detail` (deepening a skeleton on approach, always through the same agent machinery, never improvised from the skeleton); play-time truth lives in `design/overlay.json`. In play the same conductor runs the designer's play half script-side — `designer.py load-pack` at `load`, `scene --enter` before a scene opens, `end-pack` (tick → sweep → prep) and `detail` at `end`, `render_dm.py` at `save` — and never authors a designed file itself. Read `${CLAUDE_SKILL_DIR}/SKILL-design.md` the moment `new` or a `design` command is invoked.
...
| **Haiku** | `claude-haiku-4-5-20251001` | Formatting only: XP summaries, NPC attitude lines, quest one-liners, `report.md` prose |
| **Sonnet** | `claude-sonnet-5` | All DM work at a Sonnet table: narration, NPC dialogue, skill outcomes, plot decisions, combat; the designer's `--economy` tier for fan-out dossiers and their critique |
| **Opus** | `claude-opus-5-5` / `claude-fable-5-1` | `/dm:dnd new` — the whole birth runs in an Opus-tier session (generation and critique inherit it; the conductor warns otherwise); `/dm:dnd character new` pillar derivation; `design integrate` |
...
- **Every scene that consumes in-world time ends by spending it, and the next scene opens by stating the clock.** A day is spent in scenes that each cost something: a conversation, a walk across town, an audience, an afternoon in an archive. Close each one with `calendar.py -c <campaign> scene <kind>` (or `advance N minutes` for anything the table does not cover), then open the next scene with the time it is now. Both halves matter — advancing without ever saying the time hides the clock from the player, and stating the time without advancing it is how a party talks to three people, attends a meeting, researches all afternoon and it is somehow still breakfast. When a scene's cost is obvious in the fiction, say it out loud as you narrate: *"a little after two"*, *"the bells ring four as you leave"*.
...
- **A fact that reached the world through a private channel stays there.** A Sending Stone message, a closed-room agreement, a sealed letter, a whispered spell: `scripts/channels.py` records who was actually party to each one and what passed through it. Before an NPC cites a number, a plan or an order, run `channels.py -c <campaign> check "<the line>" --speaker "<who says it>"` — it answers whether that speaker could know it, instead of asking you to remember. Record the exchange the moment it happens (`channels.py fact <channel> --day N --text "..."`), because the leak always comes later, when you have forgotten the fact was ever private. This is the sharpest form of the check below: the DM wrote the fact minutes ago, so the NPC knowing it feels natural.
...
- **Mandatory source-of-knowledge check before ANY NPC says anything substantive — every NPC, whether or not they have a full entry.** Before writing the line, answer explicitly: *which specific scene, report, document, or relationship gave this NPC this fact?* If no channel exists, the NPC does not know it — surface the information a different way (an NPC who does have the channel, a document, a roll, or simply not yet) rather than have this one say it.
  ...
  **Operational specifics — troop counts, private conversations, a Sending's exact contents — are the sharpest version of this check, because the DM knows them cold and it's easy to just write them into the next NPC's mouth.** A caught example: mid-session, an NPC in a public negotiation referenced the exact number of soldiers the party had privately ordered gathered via a Sending Stone message minutes earlier — no channel existed for that NPC to know it, only the two PCs on the call did. Before an NPC cites a number, a private plan, or the content of a message the PCs sent or received off-scene, check it received that information through something the fiction actually shows, not through the DM having typed it a few lines up.
...
- **Before writing an opening scene, check who is dead.** `campaign_graph.py gone --campaign <name> --type npc` lists everyone recorded as killed, executed or destroyed, newest first. An opening beat that greets a corpse, or hands a job to someone the party buried, is the most expensive continuity error available — and the fact is always already recorded, just never in front of the scene being written. A load-opening "bang" that leans on a specific NPC's fate reads that roster first; speed there buys nothing.
...
- **This applies hardest at the session opener, and Live State Flags alone is not always enough.** A `/dm:dnd load` bang (Standard 13) is exactly the moment the urge to open strong is highest and the temptation to build it on a half-remembered fact is highest too — a session that opened this way once was built around an NPC's fate that state.md itself recorded two different ways in two different sections (a compact Live State Flags line said one thing, the actual session-log-archive entry for the event said another), and the DM wrote a whole opening scene on the wrong one before the player caught it. When a load-opening bang depends on a specific, checkable fact — an NPC's fate, who knows what, whether an event already happened — and that fact doesn't come from something read *this session*, spend the one extra Read to confirm it against the most specific source (the archived session entry beats a summary line every time) before committing it to the opening. A slower, correct opening beats a fast, wrong one; walking back a fabricated scene costs far more of the player's trust than the few seconds saved by not checking.

- **Continuity micro-save (autosave).** Unless `state.md → ## Session Flags` has `autosave: off`, keep unsaved continuity near zero so a context compaction can never cost more than a turn or two. At each natural scene boundary — a location change, the end of combat, a major NPC reveal or disposition shift — and otherwise every several turns, *silently* flush the continuity anchors: **spend the scene's time (`calendar.py scene <kind>`) and copy `calendar.py stateline` into the date line**, update `## Live State Flags` in `state.md`, append any new relationships to the campaign graph, and make sure recent beats are in the session tail. This is a lightweight write, **not** a full `/dm:dnd save` — do not rewrite `session-log.md`, do not narrate it, do not interrupt the scene. It is the same information a save captures, just kept current continuously instead of only at session end. If the optional autosave Stop hook is installed (`install_autosave_hook.py`), it will also prompt this flush on a turn cadence as a backstop — but do not wait for it; the scene-boundary habit is the primary mechanism.
...
**Development belongs to its own tab.** While a session is open the skill is read-only — a `skill_edit_guard` hook blocks edits under the skill directory, and campaign files stay writable. A finding about the tool goes to `campaigns/<name>/dev-queue.md` during play; the fix happens after `/dm:dnd end`, in a fresh tab that starts by reading that queue. A development tab's first move is `campaigns/<name>/dev-queue.md`, not `/dm:dnd load` — it needs the tool in context, not the campaign. (If a script breaks mid-play and the session cannot continue, `.runtime/allow-skill-edit` lifts the lock; delete it when the fix is in.)

**Reference modules:** For full script syntax, Read `${CLAUDE_SKILL_DIR}/SKILL-scripts.md`. For full command procedures, Read `${CLAUDE_SKILL_DIR}/SKILL-commands.md`. Load both at `/dm:dnd load`. **The moment combat starts** (the first `combat.py init` call of a scene), also Read `${CLAUDE_SKILL_DIR}/SKILL-combat.md` — reference tables (maneuvers, conditions, armor/Stealth) plus the turn-by-turn discipline (crit/kill bonus actions, opportunity attacks, never batching turns, treating the tracker as the source of truth) that catches the mistakes memory alone tends to drop mid-fight. **When designing a boss, mini-boss or set-piece encounter**, Read `${CLAUDE_SKILL_DIR}/SKILL-encounter-design.md` — the checklist is mandatory for boss-tier fights, and its application table keeps ordinary rooms light. **The moment a journey of a day or more begins**, Read `${CLAUDE_SKILL_DIR}/SKILL-travel.md` — never fast-forward overland travel into "nothing happened"; roll for it, for real, every leg. **The moment `/dm:dnd new` or any `/dm:dnd design` command is invoked**, Read `${CLAUDE_SKILL_DIR}/SKILL-design.md` — the dials, the phase loop, the blind-conductor rule and every `design` procedure live there.
