...
- **Mandatory source-of-knowledge check before any NPC with a full `npcs-full.md` entry says anything substantive (added 2026-09-14, after repeated live failures across multiple sessions — the DM's own meta-knowledge, or another character's private information, kept leaking into an NPC's mouth despite the rule above already existing).** Reading the full entry is not sufficient on its own — before writing the line, explicitly answer: *which specific scene, report, document, or relationship gave this NPC this fact, and is that channel already recorded in their entry's **Known Facts** ledger (see `templates/npcs.md`)?* If the fact isn't in the ledger and no plausible channel exists, the NPC does not know it — surface the information a different way (an NPC who does have the channel, a document, a roll, or simply not yet) rather than have this one say it. The moment an NPC learns something new in a scene — directly, by report, or by inference — add a dated line to their `Known Facts` ledger before moving on; future scenes check against that written ledger, not against DM memory of "what would make sense for them to know." This mandatory check applies to any NPC significant enough to carry a full entry; a genuine one-scene walk-on with no entry gets a lighter mental check instead of a file write.
